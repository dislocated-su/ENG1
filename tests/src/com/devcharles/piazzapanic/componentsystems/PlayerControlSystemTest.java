...
public class PlayerControlSystemTest {
  ...
  public void testProcessEntityInteractInputs(){
    ...
    assertFalse("The other cook should not have a player component and therefore no changes should be applied",
  ...
  public void testProcessEntityMovementInputs(){
    ...
    assertEquals("The cook with the player component should not have moved on the y-axis",
        0, cook1Body.body.getPosition().y, 0.001f);

    assertEquals("The position of the other cook should not have changed",
        new Vector2(50, -27), cook2Body.body.getPosition());
    ...
    kbInput. down = true;
    engine.update(1f);
    assertEquals("No change should be made on the x-axis",
        cook1Body.body.getPosition().x, cook1PreviousPosition.x, 0.001f);

    assertEquals("No change should be made on the y-axis",
        cook1Body.body.getPosition().y, cook1PreviousPosition.y, 0.001f);
  }
  ...
  public void testProcessEntitySwitchCooks(){
    ...
    assertTrue("The second cook should now have the player component",
        Mappers.player.has(cook2));
    ...
    engine.update(1f);
    assertTrue("As the process of switching cooks is still ongoing, the system variable for switching cooks should be true",
        system.changingCooks);
    ...
    engine.update(1f);
    assertTrue("The first cook should now have the player component",
        Mappers.player.has(cook1));
